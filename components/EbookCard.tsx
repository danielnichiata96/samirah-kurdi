...
export default function EbookCard({ titulo, descricao, preco, capa = '/images/ebook.svg', href }: Props) {
  ...
  <h3 className="font-semibold mb-1"><span className="font-sans">{titulo}</span></h3>
      <p className="text-sm text-zinc-600 mb-3">{descricao}</p>
        ...
        {href ? (
              isExternal ? (
                <a href={href} className="text-sm font-medium text-zinc-900 hover:underline" target="_blank" rel="noopener noreferrer">Comprar →</a>
              ) : (
                <Link href={href as any} className="text-sm font-medium text-zinc-900 hover:underline">Comprar →</Link>
              )
        ) : (
